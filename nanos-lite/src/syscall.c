...
void do_syscall(Context *c) {
  ...
  switch (a[0]) {
		...
		case SYS_yield: c->GPRx = sys_yield(c->GPR2); break;
		// case SYS_yield: c->GPRx = schedule(c->GPR2); break;
		...
		case SYS_gettimeofday: c->GPRx = sys_gettimeofday((intptr_t)c->GPR2, (intptr_t)c->GPR3); break;
