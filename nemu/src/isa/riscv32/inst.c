...
#define immCSR() do { *src1 = ZEXT(BITS(i, 19, 15), 5);  } while(0) // Note that we just use the place of src1 instead of dump out the register value
																																	 // 实际上，src1、src2只是方便看的名字，取地址操作在用srcXR()宏包装的R()宏中
...
//	case TYPE_Iz:src1R();          immIz();break;
	case TYPE_CSR: src1R(); immI(); break;
	case TYPE_CSRI: immCSR(); immI(); break;
	// bits: lo -> hi
  }
}
...
//  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, R(rd) = ((word_t)src1 < (word_t)src2 ? 1 : 0));
  ...
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(rd) = ((int64_t)(SEXT(src1, 32) * src2) >> 32));
...
//  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = ((word_t)src1) >> (imm & 31)); // only use imm[0:4]
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = (src1) >> (imm & 31)); // only use imm[0:4]
...
//  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = ((word_t)src1 < (word_t)imm ? 1 : 0)); // notice this should treat as unsigned though sign-extended
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = (src1 < imm ? 1 : 0)); // notice this should treat as unsigned though sign-extended

	// load/save
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = SEXT(Mr(src1 + imm, 1), 8)); // Notice to do SEXT there // 
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = SEXT(Mr(src1 + imm, 2), 16)); // Notice to do SEXT there
//  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = (word_t)Mr(src1 + imm, 1)); // what is zero-extend?
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + imm, 1)); // what is zero-extend?
...
#define CSRW(i) *csr_write(i)
  ...
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs   , CSR, R(rd) = ZEXT(CSRR(imm), 32), CSRW(imm) = CSRR(imm) | src1); // csrrw, rd == x0 即为 csrw
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = isa_raise_intr(R(17), s->pc)); // R(17) == a7
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , N, s->dnpc = isa_return_intr()); // R(17) == a7
