...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = src1 >> (imm & 31)); // only use imm[0:4]
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 1, s->dnpc = s->pc + imm);
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 1, s->dnpc = src1 + imm);
  ...
  // // if-else --fail
  // // // max --fail -> consider B-type prob
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B,do{s->dnpc = (src1 == src2 ? s->pc + imm : s->dnpc); printf("%x %x %x\n", imm, s->pc, s->dnpc);}while(0));
  // INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, s->dnpc = (src1 < src2 ? s->pc + imm : s->dnpc));
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, do{s->dnpc = (src1 < src2 ? s->pc + imm : s->dnpc); printf("%x %x %x\n", imm, s->pc, s->dnpc);} while(0));
  ...
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sw     , S, Mw(src1 + imm, 2, src2));
